...
const ECONOMY_TOKEN = process.env.DISCORD_TOKEN_ECONOMY;
const TARGET_GUILDS = [GUILDS.MAIN, GUILDS.STAFF].filter(id => id);

if (!ECONOMY_TOKEN) {
    console.error('❌ DISCORD_TOKEN_ECONOMY no encontrado en .env');
    process.exit(1);
}

const commands = [];

// Cargar comandos de economy
const economyPaths = ['commands/economy', 'commands/casino', 'commands/bank'];
for (const folder of economyPaths) {
...
const rest = new REST({ version: '10' }).setToken(ECONOMY_TOKEN);
