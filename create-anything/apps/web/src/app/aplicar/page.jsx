...
export default function AplicarPage() {
  ...
  const verifyRoblox = async () => {
    ...
    try {
      // Simulate Roblox API verification
      await new Promise((resolve) => setTimeout(resolve, 1500));

      setRobloxVerified(true);
      updateFormData("roblox_user_id", "123456789");
      updateFormData("roblox_avatar_url", "https://via.placeholder.com/150");
    } catch (err) {
      setError("No se pudo verificar el usuario de Roblox");
    } finally {
                ...
                {idx < STEPS.length - 1 && (
                  <div
                    className={`h-0.5 flex-1 mx-2 transition-all duration-300 ${
                      currentStep > step.id ? "bg-[#FFD700]" : "bg-white/10"
                    }`}
                  ></div>
                )}
              </div>
